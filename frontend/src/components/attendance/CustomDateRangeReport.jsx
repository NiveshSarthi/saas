...
export default function CustomDateRangeReport({ 
  ...
  const metrics = useMemo(() => {
    ...
    const complianceIssues = {
      missingPunches: records.filter(r => !r.check_out_time && r.status === 'checked_in').length,
  ...
  }, [records, allUsers, startDate, endDate, departments]);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-600 mt-1">
            {format(startDate, 'MMM dd, yyyy')} - {format(endDate, 'MMM dd, yyyy')} ({metrics.totalDays} days)
          </p>
        </div>
        ...
        <TabsContent value="monthly" className="space-y-6 mt-6">
          <MonthlyAttendanceReport
            records={isFullMonth ? records : records.filter(r => {
              const rDate = new Date(r.date);
              return rDate.getMonth() === startDate.getMonth() && 
                     rDate.getFullYear() === startDate.getFullYear();
            })}
            allUsers={allUsers}
          ...
          <div className="space-y-4">
            {Object.entries(metrics.deptProductivity)
              ...
              <div key={dept} className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border">
                ...
                <Badge className={`${
                  parseFloat(data.productivityScore) >= 85 ? 'bg-green-500' :
